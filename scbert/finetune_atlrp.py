...
class SCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.from_numpy(seq).long().to(device), int(self.label[idx])
...
if os.path.exists(ckpt_path):
    ...
    if is_master: print(f"Resumed from {start_epoch} acc={best_acc:.4f}")
...
for epoch in range(start_epoch, EPOCHS+1):
    ...
    if is_master: print(f"[Epoch {epoch}] Train loss={train_loss:.4f}, acc={train_acc:.2f}%")
...
for seq, lbl in zip(seqs, lbls):
    net.zero_grad()
    reps = net.to_tokens(seq.unsqueeze(0))
    reps.requires_grad_()
...
if is_master: print("Saved:", f"{model_name}_relevance_topbot.csv")
