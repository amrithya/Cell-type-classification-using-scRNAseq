...
wrapper_model = ModelFromEmbeddings(model)
...
for data_v, labels_v in tqdm(val_loader, desc="DeepLIFT on val"):
    ...
    with torch.no_grad():
        embedded_input = model.token_emb(data_v)
        outputs = model.to_out(embedded_input)
    ...
    embedded_input_correct = model.token_emb(data_correct)
