...
def save_ckpt(epoch, model, optimizer, scheduler, losses, model_name):
    ...
    """
    save_path = f"/data1/data/corpus/scMODEL/{model_name}_full_model_Zheng68K.pkl"
...
def save_ckpt_atlrp(epoch, model, optimizer, scheduler, losses, model_name):
    """
    Save checkpoint including epoch, model, optimizer, scheduler states and losses.
    """
    save_path = f"/data1/data/corpus/scMODEL/{model_name}_atlrp_model_Zheng68K.pkl"
    ckpt_dir = os.path.dirname(save_path)
    if not os.path.exists(ckpt_dir):
        os.makedirs(ckpt_dir)
